...
# ANOVA, Post-hoc Tests, Cohen's d Effect Sizes, PCA, and Ratio Plots
#
...
# Updated: 2026.01.16 - Added PCA analysis with toggles, improved file naming
#
...
run_anova <- FALSE   # Set FALSE to skip ANOVA entirely (saves time)
run_pca <- TRUE     # Set TRUE to run PCA analysis

# ANOVA scenario control
run_all_scenarios <- FALSE  # FALSE = 6-potency only (fast), TRUE = all 17 scenarios

# PCA configuration
pca_active_vars <- c("cluster_prominence", "cluster_shade", "correlation",
...
pca_include_evaporation <- TRUE    # Include evaporation_duration in PCA?
pca_evaporation_supplementary <- FALSE  # If TRUE, evaporation is supplementary; if FALSE, it's active
pca_scale <- TRUE  # Standardize variables (recommended for PCA with different scales)
...
# Create output folders based on what's being run
if (run_anova && run_pca) {
  output_folder_base <- paste0(date2, "_ASPS_", analysis_type, "_ANOVA_and_PCA")
} else if (run_anova) {
  output_folder_base <- paste0(date2, "_ASPS_", analysis_type, "_ANOVA")
} else if (run_pca) {
  output_folder_base <- paste0(date2, "_ASPS_", analysis_type, "_PCA")
} else {
  stop("At least one of run_anova or run_pca must be TRUE")
}
...
# For ANOVA outputs (maintain backward compatibility with existing code)
...
#===== Common configuration for both ANOVA and PCA =========================
...
if (run_pca) {
  ...
  # Prepare PCA dataset
  cat("Preparing PCA dataset...\n")

  # Build list of active variables
  pca_vars_active <- pca_active_vars

  # Add evaporation_duration if requested and not supplementary
  if (pca_include_evaporation && !pca_evaporation_supplementary && analysis_type == "texture") {
    if ("evaporation_duration" %in% colnames(df)) {
      pca_vars_active <- c(pca_vars_active, "evaporation_duration")
      cat("  Including evaporation_duration as ACTIVE variable\n")
    } else {
      cat("  WARNING: evaporation_duration not found in dataset\n")
    }
  }

  # Build list of supplementary quantitative variables
  pca_vars_supp_quant <- c()
  if (pca_include_evaporation && pca_evaporation_supplementary && analysis_type == "texture") {
    if ("evaporation_duration" %in% colnames(df)) {
      pca_vars_supp_quant <- "evaporation_duration"
      cat("  Including evaporation_duration as SUPPLEMENTARY variable\n")
    }
  }

  # Check which variables exist in the dataset
  missing_vars <- setdiff(pca_vars_active, colnames(df))
  if (length(missing_vars) > 0) {
    cat("  WARNING: Following active variables not found:", paste(missing_vars, collapse=", "), "\n")
    pca_vars_active <- intersect(pca_vars_active, colnames(df))
  }

  cat("  Active variables:", paste(pca_vars_active, collapse=", "), "\n")
  if (length(pca_vars_supp_quant) > 0) {
    cat("  Supplementary quant variables:", paste(pca_vars_supp_quant, collapse=", "), "\n")
  }

  # Prepare data for PCA - remove rows with NAs in active variables
  pca_data_complete <- df %>%
    select(all_of(c(pca_vars_active, pca_vars_supp_quant,
                    "potency_decoded", "experiment_number", "experiment_name")))

  # Count NAs before removal
  n_before <- nrow(pca_data_complete)
  na_counts <- colSums(is.na(pca_data_complete[, pca_vars_active, drop = FALSE]))

  if (any(na_counts > 0)) {
    cat("\n  NA counts in active variables:\n")
    print(na_counts[na_counts > 0])
  }

  # Remove rows with any NA in active variables
  pca_data_complete <- pca_data_complete %>%
    filter(complete.cases(.[, pca_vars_active]))

  n_after <- nrow(pca_data_complete)
  n_removed <- n_before - n_after

  cat(sprintf("\n  Observations before NA removal: %d\n", n_before))
  cat(sprintf("  Observations removed due to NA: %d\n", n_removed))
  cat(sprintf("  Observations for PCA: %d\n\n", n_after))

  if (n_after < 10) {
    cat("  ERROR: Too few observations for PCA after NA removal\n")
    run_pca <- FALSE
  }
}
...
if (run_pca) {
  ...
  # Determine column indices for FactoMineR
  active_cols <- which(colnames(pca_data_complete) %in% pca_vars_active)
  quali_cols <- which(colnames(pca_data_complete) %in% c("potency_decoded", "experiment_number", "experiment_name"))

  supp_quant_cols <- NULL
  if (length(pca_vars_supp_quant) > 0) {
    supp_quant_cols <- which(colnames(pca_data_complete) %in% pca_vars_supp_quant)
  }

  # Run PCA
  pca_result <- PCA(
    pca_data_complete,
    quanti.sup = supp_quant_cols,
    quali.sup = quali_cols,
    scale.unit = pca_scale,
    ncp = min(5, length(pca_vars_active)),  # Number of dimensions to keep
    graph = FALSE
  )

  cat("  ✓ PCA completed\n")
  cat(sprintf("  Dimensions retained: %d\n", ncol(pca_result$ind$coord)))
  cat(sprintf("  Variance explained by PC1: %.2f%%\n", pca_result$eig[1, 2]))
  cat(sprintf("  Variance explained by PC2: %.2f%%\n", pca_result$eig[2, 2]))
  cat(sprintf("  Cumulative variance PC1+PC2: %.2f%%\n", sum(pca_result$eig[1:2, 2])))
  ...
  scree_plot <- fviz_eig(pca_result, addlabels = TRUE, ylim = c(0, 50)) +
    labs(title = paste0("PCA Scree Plot - ", toupper(analysis_type), " Analysis"),
         subtitle = paste0("Active variables: ", paste(pca_vars_active, collapse = ", "))) +
    theme_bw() +
    theme(plot.title = element_text(size = 14, face = "bold"),
          plot.subtitle = element_text(size = 10))

  scree_filename <- file.path(output_folder_base,
                              paste0(date2, "_ASPS_", analysis_type, "_PCA_screeplot.png"))
  ggsave(scree_filename, scree_plot, width = 20, height = 15, units = "cm", dpi = 300)
  cat(sprintf("    Saved: %s\n", basename(scree_filename)))


  # 2. Biplot - Variables
  cat("  Creating variable biplot...\n")

  var_biplot <- fviz_pca_var(pca_result,
                             col.var = "contrib",
                             gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
                             repel = TRUE) +
    labs(title = paste0("PCA Variables - ", toupper(analysis_type)),
         subtitle = paste0("Active variables (n=", length(pca_vars_active), ")")) +
    theme_bw() +
    theme(plot.title = element_text(size = 14, face = "bold"),
          plot.subtitle = element_text(size = 10))

  var_biplot_filename <- file.path(output_folder_base,
                                   paste0(date2, "_ASPS_", analysis_type, "_PCA_biplot_variables.png"))
  ggsave(var_biplot_filename, var_biplot, width = 20, height = 18, units = "cm", dpi = 300)
  cat(sprintf("    Saved: %s\n", basename(var_biplot_filename)))


  # 3. Biplot - Individuals colored by potency
  cat("  Creating individual biplot (colored by potency)...\n")

  ind_biplot_potency <- fviz_pca_ind(pca_result,
                                     geom.ind = "point",
                                     col.ind = pca_data_complete$potency_decoded,
                                     palette = potency_colors,
                                     addEllipses = TRUE,
                                     ellipse.level = 0.95,
                                     legend.title = "Potency") +
    labs(title = paste0("PCA Individuals - ", toupper(analysis_type)),
         subtitle = paste0("Colored by potency (n=", n_after, " observations)")) +
    theme_bw() +
    theme(plot.title = element_text(size = 14, face = "bold"),
          plot.subtitle = element_text(size = 10))

  ind_biplot_potency_filename <- file.path(output_folder_base,
                                           paste0(date2, "_ASPS_", analysis_type, "_PCA_biplot_individuals_by_potency.png"))
  ggsave(ind_biplot_potency_filename, ind_biplot_potency, width = 22, height = 18, units = "cm", dpi = 300)
  cat(sprintf("    Saved: %s\n", basename(ind_biplot_potency_filename)))


  # 4. Biplot - Individuals colored by experiment
  cat("  Creating individual biplot (colored by experimenter)...\n")

  experimenter_colors <- c("ASPS_AS" = "#E7298A", "ASPS_JZ" = "#D95F02")

  ind_biplot_exp <- fviz_pca_ind(pca_result,
                                 geom.ind = "point",
                                 col.ind = pca_data_complete$experiment_name,
                                 palette = experimenter_colors,
                                 addEllipses = TRUE,
                                 ellipse.level = 0.95,
                                 legend.title = "Experimenter") +
    labs(title = paste0("PCA Individuals - ", toupper(analysis_type)),
         subtitle = paste0("Colored by experimenter (n=", n_after, " observations)")) +
    theme_bw() +
    theme(plot.title = element_text(size = 14, face = "bold"),
          plot.subtitle = element_text(size = 10))

  ind_biplot_exp_filename <- file.path(output_folder_base,
                                       paste0(date2, "_ASPS_", analysis_type, "_PCA_biplot_individuals_by_experimenter.png"))
  ggsave(ind_biplot_exp_filename, ind_biplot_exp, width = 22, height = 18, units = "cm", dpi = 300)
  cat(sprintf("    Saved: %s\n", basename(ind_biplot_exp_filename)))


  # 4b. All pairwise PC combinations - Individuals colored by potency
  cat("  Creating all pairwise PC combinations (colored by potency)...\n")

  n_pcs <- ncol(pca_result$ind$coord)

  # Generate all pairwise combinations of PCs
  pc_pairs <- combn(1:n_pcs, 2, simplify = FALSE)

  cat(sprintf("    Generating %d pairwise plots (PC combinations)\n", length(pc_pairs)))
  ...
  for (pair in pc_pairs) {
    ...
    var_x <- round(pca_result$eig[pc_x, 2], 1)
    var_y <- round(pca_result$eig[pc_y, 2], 1)

    # Create biplot for this PC pair
    biplot_pair <- fviz_pca_ind(pca_result,
                                axes = c(pc_x, pc_y),
                                geom.ind = "point",
                                col.ind = pca_data_complete$potency_decoded,
                                palette = potency_colors,
                                ...
                                legend.title = "Potency") +
      labs(title = paste0("PCA Individuals - ", toupper(analysis_type)),
           subtitle = paste0("PC", pc_x, " (", var_x, "%) vs PC", pc_y, " (", var_y, "%)"),
    ...
    pair_filename <- file.path(output_folder_base,
                               paste0(date2, "_ASPS_", analysis_type,
                                      "_PCA_biplot_individuals_by_potency_PC", pc_x, "_vs_PC", pc_y, ".png"))
    ggsave(pair_filename, biplot_pair, width = 22, height = 18, units = "cm", dpi = 300)

    cat(sprintf("      Saved: PC%d vs PC%d\n", pc_x, pc_y))
  }

  cat(sprintf("    ✓ Completed %d pairwise plots\n", length(pc_pairs)))


  # 5. Contribution plot
  cat("  Creating variable contributions plot...\n")

  contrib_plot <- fviz_contrib(pca_result, choice = "var", axes = 1:2, top = 10) +
    labs(title = paste0("Variable Contributions to PC1 and PC2 - ", toupper(analysis_type))) +
    theme_bw() +
    theme(plot.title = element_text(size = 14, face = "bold"),
          axis.text.x = element_text(angle = 45, hjust = 1))

  contrib_filename <- file.path(output_folder_base,
                                paste0(date2, "_ASPS_", analysis_type, "_PCA_contributions.png"))
  ggsave(contrib_filename, contrib_plot, width = 22, height = 15, units = "cm", dpi = 300)
  cat(sprintf("    Saved: %s\n", basename(contrib_filename)))


  # 6. Save numerical results to Excel
  cat("  Creating Excel file with PCA results...\n")

  wb_pca <- createWorkbook()

  # Sheet 1: Eigenvalues
  addWorksheet(wb_pca, "Eigenvalues")
  eig_df <- as.data.frame(pca_result$eig)
  eig_df <- cbind(Dimension = rownames(eig_df), eig_df)
  writeData(wb_pca, "Eigenvalues", eig_df, rowNames = FALSE)

  # Sheet 2: Variable coordinates
  addWorksheet(wb_pca, "Variable_Coordinates")
  var_coord <- as.data.frame(pca_result$var$coord)
  var_coord <- cbind(Variable = rownames(var_coord), var_coord)
  writeData(wb_pca, "Variable_Coordinates", var_coord, rowNames = FALSE)

  # Sheet 3: Variable contributions
  addWorksheet(wb_pca, "Variable_Contributions")
  var_contrib <- as.data.frame(pca_result$var$contrib)
  var_contrib <- cbind(Variable = rownames(var_contrib), var_contrib)
  writeData(wb_pca, "Variable_Contributions", var_contrib, rowNames = FALSE)

  # Sheet 4: Variable cos2 (quality of representation)
  addWorksheet(wb_pca, "Variable_Cos2")
  var_cos2 <- as.data.frame(pca_result$var$cos2)
  var_cos2 <- cbind(Variable = rownames(var_cos2), var_cos2)
  writeData(wb_pca, "Variable_Cos2", var_cos2, rowNames = FALSE)

  # Sheet 5: Individual coordinates (first 100 rows to avoid Excel size issues)
  addWorksheet(wb_pca, "Individual_Coordinates")
  ind_coord <- as.data.frame(pca_result$ind$coord)
  ind_coord <- cbind(
    Row = 1:nrow(ind_coord),
    Potency = pca_data_complete$potency_decoded,
    Experiment = pca_data_complete$experiment_number,
    Experimenter = pca_data_complete$experiment_name,
    ind_coord
  )
  # Limit to first 500 rows if dataset is large
  if (nrow(ind_coord) > 500) {
    writeData(wb_pca, "Individual_Coordinates",
              "NOTE: Only first 500 individuals shown (full data too large for Excel)",
              startRow = 1, startCol = 1)
    writeData(wb_pca, "Individual_Coordinates", ind_coord[1:500, ],
              startRow = 3, rowNames = FALSE)
  } else {
    writeData(wb_pca, "Individual_Coordinates", ind_coord, rowNames = FALSE)
  }

  # Save workbook
  pca_excel_filename <- file.path(output_folder_base,
                                  paste0(date2, "_ASPS_", analysis_type, "_PCA_results.xlsx"))
  saveWorkbook(wb_pca, pca_excel_filename, overwrite = TRUE)
  cat(sprintf("    Saved: %s\n", basename(pca_excel_filename)))
  ...
  cat("================================================================================\n")
  cat(sprintf("Active variables: %s\n", paste(pca_vars_active, collapse=", ")))
  if (length(pca_vars_supp_quant) > 0) {
    cat(sprintf("Supplementary variables: %s\n", paste(pca_vars_supp_quant, collapse=", ")))
  }
  cat(sprintf("Observations analyzed: %d\n", n_after))
  cat(sprintf("Variance explained by PC1-PC2: %.2f%%\n", sum(pca_result$eig[1:2, 2])))
  cat(sprintf("\nOutputs saved to: %s/\n", output_folder_base))
  cat("  - 1 scree plot\n")
  cat("  - 1 variable biplot\n")
  cat("  - 2 individual biplots (PC1 vs PC2: by potency and experimenter)\n")
  cat(sprintf("  - %d pairwise PC combination plots (all PC pairs colored by potency)\n", length(pc_pairs)))
  cat("  - 1 contribution plot\n")
  cat("  - 1 Excel results file\n")
  cat(sprintf("  TOTAL: %d plot files + 1 Excel file\n", 4 + length(pc_pairs)))
  cat("================================================================================\n\n")
}
...
if (run_anova) {
  ...
  create_anova_workbook <- function(df_filtered, scenario) {
    ...
    # Save workbook to output folder - UPDATED NAMING
    output_filename <- file.path(output_folder,
                                 paste0(date2, "_ASPS_", analysis_type, "_ANOVA_",
  ...
  create_plot <- function(df_filtered, scenario) {
    ...
    # Save to PNG file - UPDATED NAMING
    output_filename <- file.path(output_folder,
                                 paste0(date2, "_ASPS_", analysis_type, "_ANOVA_plot_",
  ...
  cat(sprintf("Library emmeans procedure\n\n"))
  ...
  # Save post-hoc workbook - UPDATED NAMING

  output_filename_posthoc <- file.path(output_folder,
                                       paste0(date2, "_ASPS_", analysis_type,
                                              "_ANOVA_posthoc_emmeans_cohensd.xlsx"))
